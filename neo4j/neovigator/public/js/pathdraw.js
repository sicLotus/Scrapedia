...
function submit() {
    ...
    if(!start) {
        error("You must give a starting article");
        return;
    }
    if(!target) {
        error("You must give a target article");
        return;
    }
    ...
    $.ajax({
        url: "http://ec2-54-246-66-235.eu-west-1.compute.amazonaws.com:8088/pathfinder/" + data,
        dataType: "text",
        success: function (data) {
            var d = eval(decodeURI(data).replace(/\+/g, " "));
            if(d) {
              if (d.length > 0 && d[0].substring) {
              error(d[0]);
            } else {
              createCheckboxes(d);
            }
